...
Company & Market Intelligence Engine. Layers 1-2 complete (SEC EDGAR + NLP transcripts).
...
uv run pytest --cov                        # Run tests (123 tests, 78% coverage)
...
- `src/atlas_intel/` — main package
  ...
  - `ingestion/` — SEC EDGAR + FMP pipelines: rate-limited HTTP clients, ticker/submission/facts/transcript sync
  ...
  - `cli.py` — Typer CLI (`atlas sync`, `atlas sync-tickers`, `atlas sync-transcripts`)
- `alembic/` — async database migrations (001-003)
...
- Endpoint: `/api/v3/earning_call_transcript/{symbol}`
...
| 3. Market data | Planned | Price data, factor exposures, sector correlations |
